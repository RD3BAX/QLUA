   ...
   while Run do 
      ...
      if ROBOT_STATE == 'В ПРОЦЕССЕ СДЕЛКИ' then
         ...
         for i=1,10 do
            ...
            Price = Transaction.Trade(CurrentDirect);
...
function KillPos(Type)
   ...
   if Type == 'BUY' then
      ...
      while Run and not Transaction.Trade('SELL') do -- Открывает SELL, тем самым закрывая BUY, если Trade('SELL') вернет TRUE, цикл прекратится
      ...
      -- Пока скрипт не остановлен и позиция не закрыта
      while Run and not Transaction.Trade('BUY') do -- Открывает BUY, тем самым закрывая SELL, если Trade('BUY') вернет TRUE, цикл прекратится
         Count = Count + 1; -- Увеличивает счетчик
         -- Если за 10 попыток не удалось закрыть позицию
         if Count == 10 then
            -- Возвращает NIL
